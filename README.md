...
- **Free with Claude Code, Codex, or a local model.** Use your existing CLI subscription, or run Ollama / llama.cpp locally — zero API cost.
...
- One of:
  ...
  - [Ollama](https://ollama.com) or [llama.cpp](https://github.com/ggml-org/llama.cpp) — first-class local model support, configurable from the admin UI
...
| **OpenAI + custom URL** | `OPENAI_BASE_URL` | Free | vLLM, OpenRouter, or any OpenAI-compatible endpoint |
...
- **Local model users** — select Ollama or llama.cpp in the admin UI, type your model ID, and optionally set a custom base URL. No API key needed.
...
- **Backend selection** — `--backend claude-code|codex|anthropic|openai|google|ollama|llamacpp`
...
| LLM | Anthropic, OpenAI, Google, Claude Code, Codex, Ollama, or llama.cpp |
